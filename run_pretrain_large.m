...
param.classnames = {'bathtub', 'bed', 'chair'};
...
param.network = {
    struct('type', 'input');
    struct('type', 'convolution', 'outputMaps', 48, 'kernelSize', 6, 'actFun', 'sigmoid', 'stride', 1);
...
save('./model/60larg/model30_l2','model');
...
save('./model/60larg/model30_l3','model');
...
param.sparse_cost = 0;
[model] = crbm(model, data_list, param);
save('./model/60larg/model30_l4','model');


[hidden_prob_h4, train_label] = propagate_data(model, data_list, 5);
save('./model/60larg/hidden_prob_h4','hidden_prob_h4');
save('./model/60larg/train_label','train_label');
...
save('./model/60larg/model30_l5','model');
save('./model/60larg/hidden_prob_h5','hidden_prob_h5');
...
save('./model/60larg/pretrained_model','model');
